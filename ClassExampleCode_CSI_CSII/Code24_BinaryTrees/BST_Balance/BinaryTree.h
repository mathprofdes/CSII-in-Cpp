 ...
 */
template <class T> int BinaryTree<T>::height() { return height(root); }
 ...
 */
template <class T> int BinaryTree<T>::CountNodes() { return CountNodes(root); }
 ...
 */
template <class T>
void BinaryTree<T>::CreateBalanceRec(TreeNode *&nodePtr, int count, int stop) {
  if (count > stop)
    return;
