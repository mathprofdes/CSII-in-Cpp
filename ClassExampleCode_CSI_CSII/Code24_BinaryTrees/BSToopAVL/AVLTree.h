...
This version uses an external node class and inherits off of a general
 ...
 */
template <class T> bool AVLTree<T>::find(const T &item) {
 ...
 */
template <class T> AVLTreeNode<T> *AVLTree<T>::findNode(const T &item) {
...
is a BST.
*/
template <class T> bool AVLTree<T>::isBST(AVLTreeNode<T> *t) {
  if (!t)
    return true;
...
//***************************************************
template <class T> bool AVLTree<T>::search(T item) {
  AVLTreeNode<T> *nodePtr = this->root;
